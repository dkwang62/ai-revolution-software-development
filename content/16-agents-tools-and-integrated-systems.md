...
That knowledge is not usually stored as a readable rulebook called _Software Engineering Best Practices_. It is learned through repeated exposure to relationships: requirements beside implementations, code beside tests, errors beside fixes, modules beside documentation, pull requests beside reviews, and architecture beside the reasons people changed it. Later training and evaluation can then push the system towards answers that compile, pass tests, follow instructions, and resemble the work competent engineers prefer. OpenAI described Codex as trained with reinforcement learning on real-world coding tasks, able to read and edit files, run test harnesses, linters, and type checkers, and iteratively run tests until it receives a passing result. [OpenAI, _Introducing Codex_](https://openai.com/index/introducing-codex/)

This explains a surprising experience from building Radix and the Personal Librarian. I did not always know to ask for a repository layer, a local catalogue, cautious file permissions, duplicate detection, test coverage, or a clean separation between search and destructive action. Codex could still propose many of those structures because they are common patterns in software engineering, not because I had explicitly taught them. The human supplied the purpose and judged the result; the agent supplied a large amount of engineering repertoire.
