...
Codex should not be understood as only one AI model. The name has had several meanings over time: first a code-focused model, later a family of coding-agent products and workflows. In this book, it means a coding-agent system built around trained AI models, project context, tools, permissions, and a supervised loop of action and feedback. The model supplies general programming knowledge learned during training. The project files, instructions, build results, tests, and screenshots supply current context. Editing tools, terminal commands, Git, and verification steps allow the system to act inside a software project. Codex can adapt within a task by reading context and responding to tool results, but that is not the same as permanently retraining the underlying model.
